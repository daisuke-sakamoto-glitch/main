...
# 家電紹介に関連する投稿かどうかを判定するキーワード
RELEVANCE_KEYWORDS = [
    "レビュー", "買ってよかった", "買い替え", "おすすめ", "比較",
    "購入", "使ってみ", "使い心地", "機能", "性能", "コスパ",
    "神家電", "最強", "導入", "新型", "新モデル", "最新",
    "紹介", "開封", "感想", "メリット", "デメリット",
    "便利すぎ", "QOL", "時短", "節電", "省エネ",
    "型落ち", "セール", "値下げ", "コスパ最強",
]

# 明らかに無関係な投稿を除外するキーワード
EXCLUDE_KEYWORDS = [
    "冷蔵庫にあるもので", "冷蔵庫の中身", "冷蔵庫開けたら",
    "冷蔵庫にビール", "冷蔵庫から出した", "冷蔵庫に入れ",
    "洗濯機回し", "掃除機かけ",
    "歌ってみた", "踊ってみた", "ゲーム実況", "配信",
    "プレゼント企画", "フォロー&RT", "懸賞",
]


def is_relevant_post(text: str) -> bool:
    """投稿が家電紹介に関連するかどうかを判定する。"""
...
def search_tweets(bearer_token: str, query: str, max_results: int = 20,
    ...
    for tweet in data["data"]:
        ...
        # 家電紹介に関連する投稿のみ採用
        if not is_relevant_post(tweet_text):
            continue
